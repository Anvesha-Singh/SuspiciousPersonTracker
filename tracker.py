...
class App(QWidget):
    ...
    def __init__(self):
        ...
        # Initialize YOLO model for weapon detection
        #self.yolo_model = YOLO('best.pt')
        
        device = torch.device("cpu")
        self.yolo_model = YOLO("best.pt").to(device)
    ...
    def record(self):  #video recording
        ...
        while(self.rec):
            ...
            cv2.waitKey(30)                    #no delay between frames
    ...
    # Modify image_process method for weapon detection
        ...
        for result in results:
            ...
            for pos, detection in enumerate(detections):
                if conf[pos] >= 0.5:  # Confidence threshold
                    ...
                    detection_center_X = int((xmin + xmax) / 2) - (img.shape[1] // 2)
                    detection_center_Y = int((ymin + ymax) / 2) - (img.shape[0] // 2)
                    label = f"{classes[int(cls[pos])]} {conf[pos]:.2f}" 
                    
                    # Draw a bounding box around the detected object
                    color = (0, int(cls[pos]), 255)  # Using class id to determine color
                    cv2.rectangle(img, (int(xmin), int(ymin)), (int(xmax), int(ymax)), color, 2)
                    cv2.putText(img, label, (int(xmin), int(ymin) - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, color, 1, cv2.LINE_AA)

                    # Optionally, you can draw a cross at the center of the bounding box
                    center_x = int((xmin + xmax) / 2)
                    center_y = int((ymin + ymax) / 2)
                    cv2.drawMarker(img, (center_x, center_y), color=(0, 0, 255), markerType=cv2.MARKER_CROSS, markerSize=10, thickness=2)

                    break  # Exit after first detection (if only one object is tracked at a time)

        if weapon_detected:
            self.face_detected = True
            self.empty_frame_number = self.max_empty_frame
            self.target_locked = abs(detection_center_X) < self.max_target_distance and abs(detection_center_Y) < self.max_target_distance
            print(detection_center_X)
            self.calculate_camera_move(detection_center_X, detection_center_Y)
            return img
        else:
    ...
    def calculate_camera_move(self, distance_X, distance_Y):
        ...
        if(self.InvertPan): #handle inverted pan
            #self.target_pan = (((self.target_pan + 300) / 600) * 180)
            self.target_pan = self.min_pan + ((self.target_pan +300) * (self.max_pan - self.min_pan)) / (600)
            self.target_pan -= distance_X * self.PanSensivity
            ...
            self.target_pan += distance_X * self.PanSensivity
            self.target_pan = (((self.target_pan + 300) / 600) * 180)/4
        ...
        if(self.InvertTilt): #handle inverted tilt
            ...
            if(self.target_tilt>self.max_tilt):
                ...
                self.target_tilt = self.min_tilt
        print("\n" + "pan " + str(self.target_pan))
        print("\n" + "tilt " + str(self.target_tilt))
